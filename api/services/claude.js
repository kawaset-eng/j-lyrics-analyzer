...
/**
 * Claude API を使って邦楽歌詞の分析・解釈・背景情報を生成する
 * @param {string|null} lyrics - Lyrics.ovhから取得した日本語歌詞（取得できなかった場合はnull）
...
async function analyzeLyrics(lyrics, title, artist, requestId) {
  ...
  const prompt = hasLyrics
    ? `あなたは邦楽の歌詞分析と日本語表現の専門家です。以下の曲の歌詞について、JSON形式で回答してください。
...
{
  "interpretation": "歌詞の詳しい分析（800-1200字程度）。以下の内容を含めてください：\n- 曲全体のテーマとメッセージ\n- 重要な表現やフレーズの意味（5-7個のフレーズを取り上げて詳しく解説）\n- 比喩表現、言葉遊び、韻の使い方の説明\n- 歌詞に込められた文化的背景や時代性\n- アーティストの世界観や独特の表現技法\n各フレーズは「"〇〇"」のように引用符で囲んで示してください。",
  "background": "曲の背景情報（200-300字程度）。制作経緯、リリース時期、チャート成績、タイアップ、社会的影響などを説明してください。",
  "artistInfo": "アーティストの紹介（200-300字程度）。経歴、音楽スタイル、代表曲、J-POPシーンへの影響などを説明してください。"
}`
    : `あなたは邦楽の歌詞分析と日本語表現の専門家です。以下の曲について、JSON形式で回答してください。
...
{
  "lyrics": "この曲の日本語歌詞の完全な全文。セクション名（[Verse 1], [Chorus] など）を含め、原文の改行を保持してください。歌詞は省略せず全て記載してください。",
  "interpretation": "歌詞の詳しい分析（800-1200字程度）。以下の内容を含めてください：\n- 曲全体のテーマとメッセージ\n- 重要な表現やフレーズの意味（5-7個のフレーズを取り上げて詳しく解説）\n- 比喩表現、言葉遊び、韻の使い方の説明\n- 歌詞に込められた文化的背景や時代性\n- アーティストの世界観や独特の表現技法\n各フレーズは「"〇〇"」のように引用符で囲んで示してください。",
  "background": "曲の背景情報（200-300字程度）。制作経緯、リリース時期、チャート成績、タイアップ、社会的影響などを説明してください。",
  "artistInfo": "アーティストの紹介（200-300字程度）。経歴、音楽スタイル、代表曲、J-POPシーンへの影響などを説明してください。"
}`;
...
async function chatAboutSong(songContext, messages, userMessage, requestId) {
  const systemPrompt = `あなたは邦楽の歌詞分析の専門家です。ユーザーが現在閲覧している曲について質問に答えてください。
...
分析:
${songContext.interpretation || "(分析なし)"}
=== 曲情報ここまで ===`;
